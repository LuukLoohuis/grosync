...
-- 11. Purchase history tabel (suggesties)
...
CREATE POLICY "Owner full access" ON public.purchase_history FOR ALL
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Shared list access" ON public.purchase_history FOR ALL
  USING (user_has_shared_list(user_id))
  WITH CHECK (user_has_shared_list(user_id));

-- 12. Realtime inschakelen
