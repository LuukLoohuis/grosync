      ...
      <SheetContent side="bottom" className="mx-auto flex max-h-[92vh] [@supports(height:100dvh)]:max-h-[92dvh] max-w-lg flex-col gap-0 rounded-t-[20px] p-0">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-4">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            >
              <Tags className="h-3 w-3" /> Categorie
            </button>
          </div>

          <div className="mt-4 flex items-center justify-between">
            <h3 className="font-display text-[0.9375rem] font-bold text-foreground">Ingrediënten</h3>
          ...
          {shown?.sourceUrl && (
            ...
            >
              <ExternalLink className="h-3.5 w-3.5" /> Origineel bekijken
            </a>
          )}
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">
            ...
            <Button variant="outline" className="h-auto min-h-12 flex-col gap-1 px-1 py-2 text-[11px] font-medium" onClick={() => act(onMacros)}>
              <Flame className="h-4 w-4" /> Voedingswaarden
            </Button>
            <Button
              variant="outline"
