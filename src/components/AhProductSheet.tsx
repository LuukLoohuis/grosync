...
const AhProductSheet = ({ item, onClose }: AhProductSheetProps) => {
  ...
  const current: AhAlternative | null = shown?.ahProduct
    ? {
        ...
        priceBefore: shown.priceBefore != null ? Math.round((shown.priceBefore / shown.ahProduct.quantity) * 100) / 100 : null,
  ...
  const applyChoice = async () => {
    ...
    await applyAhMatches([{
      ...
      priceBefore: selected.priceBefore != null ? Math.round(selected.priceBefore * quantity * 100) / 100 : null,
