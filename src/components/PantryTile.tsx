...
import { useDragToTrash } from '@/components/DragToTrash';
...
interface PantryTileProps {
  ...
  bijwerken?: boolean;
  onOpen: () => void;
  /** Vasthouden en naar de prullenbak slepen gooit weg. */
  onRemove: () => void;
}
 ...
 */
const PantryTile = ({ item, bijwerken = false, onOpen, onRemove }: PantryTileProps) => {
  const op = item.quantity === 0;
  const staat = op ? 'op' : item.low ? 'bijna op' : null;
  const { handvatten, wordtGesleept } = useDragToTrash(item.name, onRemove);
